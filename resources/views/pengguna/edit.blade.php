...
<div class="container-fluid">
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('pengguna.update', $pengguna->PenggunaID) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control @error('NamaPengguna') is-invalid @enderror" 
                            id="NamaPengguna" name="NamaPengguna" value="{{ old('NamaPengguna', $pengguna->NamaPengguna) }}" required>
                        @error('NamaPengguna')
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        <label for="KataSandi" class="form-label">Kata Sandi <small class="text-muted">(Kosongkan jika tidak ingin mengubah)</small></label>
                        <input type="password" class="form-control @error('KataSandi') is-invalid @enderror" 
                            id="KataSandi" name="KataSandi">
                        @error('KataSandi')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-md-6">
                        ...
                        <input type="password" class="form-control @error('KonfirmasiKataSandi') is-invalid @enderror" 
                            id="KonfirmasiKataSandi" name="KonfirmasiKataSandi">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('Peran') is-invalid @enderror" id="Peran" name="Peran" required>
                        ...
                        <option value="Kasir" {{ (old('Peran', $pengguna->Peran) == 'Kasir') ? 'selected' : '' }}>Kasir</option>
                        <option value="Manager" {{ (old('Peran', $pengguna->Peran) == 'Manager') ? 'selected' : '' }}>Manager</option>
                    </select>
                    @error('Peran')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
